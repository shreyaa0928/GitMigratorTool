...
class GitHubMigrator(BaseMigrator):
    ...
    def push_branches(self, branches: list, source_clone_url: str) -> dict:
        """Migrate branches using pure GitHub API - no git CLI. Works on Render."""
        if not branches:
            return {"migrated": 0}
        migrated = 0
        errors = []
    ...
    def push_tags(self, tags: list, source_clone_url: str) -> dict:
        """Migrate tags using pure GitHub API - no git CLI. Works on Render."""
        if not tags:
            return {"migrated": 0}
        migrated = 0
        errors = []
        for tag in tags:
            try:
                ...
                errors.append(f"{tag['name']}: {str(e)[:120]}")
